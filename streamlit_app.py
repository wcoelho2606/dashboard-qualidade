...
# =======================================================
    ...
    if not lista_aqs:
        st.warning("Nenhuma AQ cadastrada.")
        st.stop()

    aq_selecionada = st.selectbox(
        "Selecione a AQ",
        lista_aqs
    )

    item_aq = df_alertas[
        df_alertas["id"] == aq_selecionada
    ].iloc[0]

    st.info(
        f"""
        AQ: {item_aq['id']} |
        Produto: {item_aq['produto']} |
        Defeito: {item_aq['defeito']}
        """
    )

    st.markdown("---")

    st.subheader("Fluxo da Tratativa")

    historico_etapas = (
        supabase
        .table("tratativas")
        .select("*")
        .eq("id_alerta", aq_selecionada)
        .order("data_registro", desc=False)
        .execute()
    )

    df_etapas = pd.DataFrame(historico_etapas.data)

    etapa_atual = ""

    if not df_etapas.empty:
        etapa_atual = df_etapas.iloc[-1]["etapa"]
...
for i, etapa in enumerate(etapas_fluxo):
    ...
    st.subheader("Adicionar Nova Tratativa")
    ...
    with st.form("form_tratativa"):
        ...
        if salvar:
            ...
            try:
                ...
                supabase.table("tratativas").insert({

                    "id_alerta": aq_selecionada,
                    "etapa": etapa,
    ...
    if not df_hist.empty:
        ...
        st.info(
            "Nenhuma tratativa cadastrada para esta AQ."
        )
