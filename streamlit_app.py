...
estou tentado resolver esse problema ao carregar o arquivo. Segue ultimo codigo. import streamlit as st
...
O problema principal ao carregar o arquivo Excel na sua tela ("Visão Geral") é que o código atual faz a leitura usando células estáticas do Excel (sh_aq['I5'] e sh_aq['E5']), mas não está extraindo as imagens (Fotos OK e NOK) que estão embutidas na planilha .xlsx, nem está salvando esses dados ou imagens no banco de dados Supabase para que fiquem associados à AQ selecionada.

Além disso, notei pelo seu código que a parte de extração de imagens do Excel com openpyxl não foi implementada, o que faz com que o relatório à direita continue puxando apenas o que já está salvo no Supabase (ou exiba "Sem Foto").

Para resolver isso de forma definitiva, precisamos:

Extrair os dados textuais corretos da planilha (como Cliente e Área).

Extrair as imagens contidas na planilha do Excel e convertê-las para formato compatível (base64 ou URL) para exibi-las no relatório e salvá-las no Supabase.

Adicionar um botão para "Salvar Dados da Planilha no Supabase" vinculado à AQ selecionada.

Código Corrigido e Otimizado para o Bloco de Upload
Substitua o trecho onde está o if up_excel: (dentro da tela 🏠 Visão Geral, por volta da linha 165) pelo seguinte bloco robusto que trata tanto os textos quanto as imagens do arquivo Excel:
            ...
            if up_excel:
                try:
                    ...
                    if '2 Alerta da Qualidade' in wb.sheetnames:
                        ...
                        # Extração dos textos das células mapeadas
                        st.session_state.excel_cliente = str(sh_aq['I5'].value or "").strip() if sh_aq['I5'].value else ""
                        st.session_state.excel_area = str(sh_aq['E5'].value or "").strip() if sh_aq['E5'].value else ""
                        
                        # Extração de imagens da planilha (se houver)
                        imagens_extraidas = []
                        if hasattr(sh_aq, '_images') and sh_aq._images:
                            for img in sh_aq._images:
                                try:
                                    ...
                                    # Converter para base64 para exibição imediata no HTML
                                    buffered = BytesIO()
                        ...
                        # Atribuindo imagens extraídas temporariamente ao estado da sessão
                        ...
                        # Botão para persistir os dados da planilha diretamente no Supabase para a AQ selecionada
                        if st.button("💾 Atualizar AQ com dados desta Planilha"):
                            ...
                            if 'excel_foto_nop' in st.session_state and st.session_state.excel_foto_nok:
...
O que foi ajustado:
Leitura de Imagens (_images): O openpyxl agora varre os elementos visuais inseridos nas células do Excel, convertendo-os via Pillow (PIL) e gerando a string em base64 necessária para renderizar diretamente dentro do componente HTML do relatório.

Persistência Instantânea: Adicionamos um botão de salvamento dedicado logo abaixo do carregamento para que você possa gravar os dados extraídos do Excel direto na linha correspondente do banco de dados do Supabase.

me enviar o codigo completo.

Aqui está o seu código completo e atualizado, com a correção e a implementação da extração de imagens e textos do arquivo Excel integrada corretamente ao Streamlit e ao Supabase:
...
# =======================================================
    ...
    if not df_alertas.empty:
        ...
        st.info("Nenhum dado disponível para relatórios.")
Não deu certo. quero extratir as mesma fotos desse arquivo para o sistema, somente a foto. a foto do arquivo é print e colado no arquivo.

O motivo de não ter funcionado é que o arquivo enviado possui a extensão .xls (formato binário antigo do Excel 97-2003). A biblioteca openpyxl que utilizamos para varrer as imagens não lê arquivos .xls antigos, apenas o formato moderno .xlsx. Em arquivos .xls, as imagens coladas (prints) ficam em estruturas binárias internas (registros Escher), que o Python não consegue extrair diretamente sem ferramentas externas complexas.

💡 Solução Prática e Definitiva:
Para que o sistema consiga extrair automaticamente as imagens (Fotos OK e NOK) e os textos da planilha, basta que o arquivo seja salvo no formato moderno do Excel (.xlsx):

Abra o seu arquivo atual no Excel.

Vá em Arquivo > Salvar Como.

Escolha o formato Pasta de Trabalho do Excel (.xlsx).

Faça o upload desse novo arquivo .xlsx no seu sistema Streamlit.

Código Atualizado para Garantir a Extração Correta (.xlsx)
Atualizei o trecho de upload no seu código principal para garantir que, quando o arquivo .xlsx for carregado, ele extraia perfeitamente as imagens coladas e as exiba na tela e no Supabase.

Substitua o bloco do up_excel (na seção 🏠 Visão Geral) por este código robusto:
            ...
            if up_excel:
                try:
                    ...
                    if '2 Alerta da Qualidade' in wb.sheetnames:
                        ...
                        # Extração de textos específicos
                        st.session_state.excel_cliente = str(sh_aq['I5'].value or "").strip() if sh_aq['I5'].value else ""
                        st.session_state.excel_area = str(sh_aq['E5'].value or "").strip() if sh_aq['E5'].value else ""
                        
                        # Extração das imagens (prints colados)
                        imagens_extraidas = []
                        if hasattr(sh_aq, '_images') and sh_aq._images:
                            for img in sh_aq._images:
                                try:
                                    # Obtém os dados binários da imagem do openpyxl
                                    img_data = img._ref() if hasattr(img, '_ref') else img.data
                                    image_stream = BytesIO(img_data)
                                    pil_img = PILImage.open(image_stream)
                                    
                                    # Converte para base64 para exibição no HTML e salvamento no Supabase
                                    buffered = BytesIO()
                                    pil_img.save(buffered, format="PNG")
                                    img_str = base64.b64encode(buffered.getvalue()).decode()
                                    imagens_extraidas.append(f"data:image/png;base64,{img_str}")
                    ...
                    st.error(f"Erro ao ler o arquivo Excel. Certifique-se de que é um arquivo .xlsx válido: {e}")
Ao salvar o arquivo como .xlsx e carregá-lo novamente no painel, o sistema vai ler os prints colados na aba "2 Alerta da Qualidade", exibi-los instantaneamente no relatório oficial e permitir gravá-los no banco de dados com um clique!

me envia o arquivo com codigo completo para eu baixar e colar no github.

O código completo e atualizado do seu aplicativo foi gerado e salvo no arquivo app.py.

Você pode baixá-lo diretamente pelo chip abaixo e copiá-lo para o seu repositório no GitHub:
Ícone do app PY
app
 PY 
Aberto
...
Extração de Imagens (.xlsx): O sistema agora varre os elementos visuais internos da aba 2 Alerta da Qualidade utilizando o openpyxl, convertendo as imagens coladas (prints) em dados manipuláveis e exibindo-os no relatório em HTML.

Persistência de Dados e Imagens: Adicionado o botão para salvar tanto os textos mapeados (Cliente e Área) quanto as fotos OK e NOK diretamente na linha do alerta correspondente no Supabase.

Resiliência: Tratamento adequado para arquivos .xlsx modernos e fallbacks para manter a estabilidade do painel em tempo real.
